...
this.sound.play();}}});EntityButtonBlackmoon=EntityButton.extend({size:{x:35,y:40},animSheet:new ig.AnimationSheet('media/img/button-blackmoon.png',35,40),update:function(){if(ig.input.pressed('click')&&this.inFocus()){if(ig.Sound.enabled)
    this.sound.play();}}});EntityButtonStart=EntityButton.extend({size:{x:181,y:60},animSheet:animSheetList.buttonStart[_LANG_CODE],update:function(){if(ig.input.pressed('click')&&this.inFocus()){if(ig.Sound.enabled){this.sound.play();}
...
ig.game.storage.set('rogers-audio',ig.Sound.enabled);this.currentAnim=this.anims[(ig.Sound.enabled).toString()];if(ig.Sound.enabled){ig.music.play();this.sound.play();}
    ...
    this.sound.play(); window.open('https://www.facebook.com/CaptainRogersChronicles','_blank')}}});EntityButtonBack=EntityButton.extend({size:{x:186,y:58},animSheet:animSheetList.buttonBack[_LANG_CODE],update:function(){if(ig.input.pressed('click')&&this.inFocus()){if(ig.Sound.enabled)
